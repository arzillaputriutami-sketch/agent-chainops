"""FastAPI boundary for ChainOps Commander."""
...
if FastAPI:
    ...
    def network_status():
        return {"status": "ok", "endpoint": "/api/network/status", "project": PROJECT_NAME, "demo": True}

    @app.get("/api/network/latency")
    def network_latency():
        return {"status": "ok", "endpoint": "/api/network/latency", "project": PROJECT_NAME, "demo": True}

    @app.get("/api/nodes")
    def list_nodes():
        return {"status": "ok", "endpoint": "/api/nodes", "project": PROJECT_NAME, "demo": True}
else:
    app = None
...
if FastAPI and analyze_domain:
    ...
    def domain_signals():
        return {"product": 'Agent ChainOps', "signals": DOMAIN_SIGNALS, "actions": DOMAIN_ACTIONS}
    ...
    @app.get("/domain/demo")
    def domain_demo():
        return {"product": 'Agent ChainOps', "reports": batch_domain_report()}
